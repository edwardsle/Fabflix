...
public class CheckoutServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			{
				PreparedStatement stm = con.prepareStatement("SELECT * FROM creditcards WHERE firstName = ? AND lastName = ? AND id = ? AND expiration = ? LIMIT 1");
			...
			*/
			...
			if (true)
			{
				...
				PreparedStatement stmInsert = con.prepareStatement("INSERT INTO sales(customerId, movieId, saleDate) VALUES (?, ?, ?)");
				PreparedStatement stmSaleId = con.prepareStatement("SELECT sales.id, sales.movieId, movies.title from sales, movies WHERE sales.movieId = movies.id AND customerId = ? AND movieId = ? ORDER BY id DESC LIMIT 1");
